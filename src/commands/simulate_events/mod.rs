...
impl EventGenerator {
    ...
    pub fn generate_event(&mut self) -> TallyEvent {
        ...
        match event_type {
            ...
            "renewed" => {
                // Calculate keeper fee as 0.5% of amount (50 bps)
                let keeper_fee = amount.saturating_mul(50) / 10000;
                // Use a mock keeper address (could be made configurable)
...
impl SimulationStats {
    fn record_event(&mut self, event: &TallyEvent) {
        ...
        let event_type = match event {
            ...
            TallyEvent::SubscriptionReactivated(_) => "subscription_reactivated",
            TallyEvent::Renewed(_) => "renewed",
            TallyEvent::Canceled(_) => "canceled",
            TallyEvent::SubscriptionClosed(_) => "subscription_closed",
            TallyEvent::PaymentFailed(_) => "payment_failed",
            TallyEvent::PlanStatusChanged(_) => "plan_status_changed",
            TallyEvent::ConfigInitialized(_) => "config_initialized",
            TallyEvent::MerchantInitialized(_) => "merchant_initialized",
            TallyEvent::PlanCreated(_) => "plan_created",
            TallyEvent::ProgramPaused(_) => "program_paused",
            TallyEvent::ProgramUnpaused(_) => "program_unpaused",
            TallyEvent::LowAllowanceWarning(_) => "low_allowance_warning",
            TallyEvent::FeesWithdrawn(_) => "fees_withdrawn",
            TallyEvent::DelegateMismatchWarning(_) => "delegate_mismatch_warning",
            TallyEvent::ConfigUpdated(_) => "config_updated",
            TallyEvent::MerchantTierChanged(_) => "merchant_tier_changed",
            TallyEvent::PlanTermsUpdated(_) => "plan_terms_updated",
            TallyEvent::TrialStarted(_) => "trial_started",
            TallyEvent::TrialConverted(_) => "trial_converted",
        };
        *self
            .events_by_type
...
impl EventSimulator {
    ...
    fn serialize_event_with_discriminator(event: &TallyEvent) -> Result<Vec<u8>> {
        ...
        // Only serialize events that the simulator actually generates
        // Other events are tracked in stats but not serialized for output
        let (event_name, event_data) = match event {
            ...
            // All other event types are not generated by the simulator
            _ => return Err(anyhow!("Event type not supported by simulator")),
        };
